...
const GlossariesContract = require('../js/GlossariesContract.js');
const GlossariesUtility = require('../js/GlossariesUtility.js');
...
describe('Glossaries', function () {
    ...
    let util = new GlossariesUtility(contractConfig.account, bc);
    let contract = new GlossariesContract(contractConfig.account, bc);
    let dmpusers = null;
    ...
    describe('#Version', function () {
        it('glossaries', async () => {
            ...
            await contract.upsertGlossary('d1', 'alice', 'laura', 222, 3, 'data1', alice.permission);
            await contract.upsertGlossary('d1', 'alice', 'laura', 222, 3, 'data3', alice.permission)
                .should.be.rejectedWith('403. Version too old');
            await contract.upsertGlossary('d1', 'alice', 'laura', 222, 2, 'data3', alice.permission)
                .should.be.rejectedWith('403. Version too old');
            await contract.upsertGlossary('d1', 'alice', 'laura', 222, 4, 'data3', alice.permission)
    ...
    describe('#Glossaries', function () {
        it('should find glossary by sender or receiver', async () => {
            ...
            await contract.upsertGlossary('d1', 'alice', 'laura', 111, 1, 'data1', alice.permission);
            await contract.upsertGlossary('d2', 'laura', 'david', 222, 2, 'data2', laura.permission);
            await contract.upsertGlossary('d3', 'alice', 'laura', 333, 3, 'data3', alice.permission);
            await contract.upsertGlossary('d4', 'laura', 'david', 444, 4, 'data4', laura.permission);
            ...
            checkResult(await util.getGlossariesBySender('laura'));
            checkResult(await util.getGlossariesByReceiver('david'));
        });

        it('should find glossary by update_at', async () => {
            const alice = await tools.makeAccount(bc, 'alice');
            await contract.upsertGlossary('d1', 'alice', 'r1', 111, 1, 'data1', alice.permission);
            await contract.upsertGlossary('d2', 'alice', 'r2', 222, 2, 'data2', alice.permission);
            await contract.upsertGlossary('d3', 'alice', 'r3', 333, 3, 'data3', alice.permission);
            await contract.upsertGlossary('d4', 'alice', 'r4', 444, 4, 'data4', alice.permission);
            const res = await util.getGlossariesByUpdateAt(222, 333 /* + 1*/);
            assert.lengthOf(res, 2);
            {
                const d = res[0];
                assert.ok(d);
        ...
        it('should modify glossary by owner (sender)', async () => {
            const alice = await tools.makeAccount(bc, 'alice');
            await contract.upsertGlossary('d1', 'alice', 'laura', 111, 222, 'data', alice.permission);
            await contract.upsertGlossary('d1', 'alice', 'john', 333, 444, 'data5', alice.permission);
            const d = await util.getGlossaryById('d1')
            {
                assert.ok(d);
            ...
            await contract.upsertGlossary('d1', 'laura', 'alice', 555, 666, 'data', alice.permission)
                .should.be.rejectedWith("missing authority of laura");

            const laura = await tools.makeAccount(bc, 'laura');
            await contract.upsertGlossary('d1', 'laura', 'alice', 111, 222, 'data', laura.permission)
        ...
        it('should not remove unknown glossary', async () => {
            const alice = await tools.makeAccount(bc, 'alice');
            await expect(contract.removeGlossary('d1', 'alice', alice.permission))
                .be.rejectedWith("404. Glossary not found");
        });
        ...
        it('should not remove foreign glossary', async () => {
            ...
            const laura = await tools.makeAccount(bc, 'laura');
            await contract.upsertGlossary('d1', 'alice', 'laura', 111, 222, 'data', alice.permission);
            await expect(contract.removeGlossary('d1', 'alice', laura.permission))
                .be.rejectedWith("missing authority of alice");
            await expect(contract.removeGlossary('d1', 'laura', laura.permission))
        ...
        it('should remove glossary', async () => {
            const alice = await tools.makeAccount(bc, 'alice');
            await contract.upsertGlossary('d1', 'alice', 'laura', 111, 222, 'data', alice.permission);
            expect(await util.isGlossaryExists('d1')).be.true;
            await contract.removeGlossary('d1', 'alice', alice.permission);
            expect(await util.isGlossaryExists('d1')).be.false;
        });
    });
